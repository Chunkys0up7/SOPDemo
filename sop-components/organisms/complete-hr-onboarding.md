---
id: organism-complete-hr-onboarding
type: organism
version: 2.0.0
title: Complete HR Onboarding Workflow
tags: [hr, onboarding, comprehensive]
composedOf:
  - atom-emergency-contact
  - atom-benefits-overview
  - atom-confidentiality-agreement
  - molecule-benefits-enrollment
  - molecule-compliance-training
---

# Complete HR Onboarding Workflow

## Overview

This comprehensive workflow covers all HR-related onboarding activities for new employees, from paperwork to benefits enrollment to compliance training.

## Pre-Start (Week Before Start Date)

### HR Preparation Checklist
- [ ] Offer letter signed and returned
- [ ] Background check completed
- [ ] Reference checks completed
- [ ] I-9 documentation requested
- [ ] Welcome email sent with first-day details
- [ ] Employee file created in HRIS
- [ ] Badge requested from Security
- [ ] IT notified of start date

---

## Day 1: Welcome and Orientation (3-4 hours)

### Morning Session (9:00 AM - 12:00 PM)

**9:00 - 9:30: Welcome and Paperwork**

HR reviews and collects:
- [ ] I-9 documentation (original documents)
- [ ] W-4 federal tax withholding
- [ ] State tax withholding forms
- [ ] Direct deposit authorization
- [ ] Emergency contact information

#### Emergency Contact Form

{{include: atom-emergency-contact}}

**9:30 - 10:00: Legal Agreements**

Employee reviews and signs:
- [ ] Employee handbook acknowledgment
- [ ] Confidentiality agreement
- [ ] Code of conduct
- [ ] At-will employment acknowledgment
- [ ] Background check authorization (if not already done)

#### Confidentiality Agreement

{{include: atom-confidentiality-agreement}}

**10:00 - 10:15: Break**

**10:15 - 11:00: Company Overview**

Presentation covering:
- Company history and mission
- Organizational structure
- Products and services
- Client/customer base
- Recent achievements and news
- Strategic goals

**11:00 - 11:45: Benefits Overview**

{{include: atom-benefits-overview}}

**11:45 - 12:00: Benefits Q&A**

### Lunch (12:00 - 1:00 PM)
- Hosted lunch with team
- Informal getting-to-know-you time
- Manager or team lead attends

### Afternoon Session (1:00 - 5:00 PM)

**1:00 - 2:00: Workplace Policies**

Review of:
- Working hours and attendance
- PTO request procedures
- Sick leave policies
- Remote work guidelines
- Communication expectations
- Dress code
- Use of company resources

**2:00 - 3:00: HRIS and Payroll Systems**

Hands-on training:
- [ ] Login to employee self-service portal
- [ ] Review personal information
- [ ] Set up direct deposit
- [ ] Learn how to request time off
- [ ] Submit timesheets (if applicable)
- [ ] Access pay stubs
- [ ] Update tax withholdings

**3:00 - 3:15: Break**

**3:15 - 4:30: Start Compliance Training**

{{include: molecule-compliance-training}}

**Note**: Complete remaining training by end of Week 1

**4:30 - 5:00: Day 1 Wrap-up**
- Review completed checklist
- Address any questions
- Provide access credentials
- Schedule follow-up meetings

---

## Week 1: Benefits and Compliance

### Days 2-5: Benefits Enrollment

{{include: molecule-benefits-enrollment}}

**Daily Goals**:
- **Day 2**: Research and compare benefit options
- **Day 3**: Complete medical, dental, vision enrollment
- **Day 4**: Set up 401(k) and other financial benefits
- **Day 5**: Submit all documentation, schedule benefits meeting

### Days 2-5: Compliance Training Completion

Continue working on compliance training modules:
- [ ] Module 1: Code of Conduct
- [ ] Module 2: Confidentiality & Data Protection
- [ ] Module 3: Anti-Discrimination & Harassment
- [ ] Module 4: Workplace Safety
- [ ] Module 5: Information Security

**Deadline**: End of Week 1

### End of Week Meeting (Friday, Week 1)

**30-minute HR check-in**:
- [ ] Verify benefits enrollment completed
- [ ] Confirm training modules finished
- [ ] Review any outstanding paperwork
- [ ] Answer questions
- [ ] Collect feedback on onboarding experience

---

## Week 2-4: Integration and Follow-up

### Week 2

**HR Activities**:
- [ ] Benefits confirmation meeting (15 mins)
- [ ] Verify insurance cards received
- [ ] Check 401(k) deduction on first paycheck
- [ ] Department-specific training assigned (if applicable)

**Employee Activities**:
- [ ] Attend team meetings
- [ ] Complete role-specific training
- [ ] Shadow team members
- [ ] Begin initial work assignments

### Week 3

**HR Activities**:
- [ ] Mid-point check-in (15 mins)
- [ ] Address any benefit or payroll issues
- [ ] Verify training completion in system
- [ ] Collect onboarding feedback survey

### Week 4

**HR Activities**:
- [ ] 30-day review meeting (30 mins)
- [ ] Performance expectations review
- [ ] Goal setting for 90-day review
- [ ] Address any concerns
- [ ] Update employee file with completed documents

**Manager Activities**:
- [ ] Formal 30-day performance discussion
- [ ] Set clear goals and expectations
- [ ] Provide feedback
- [ ] Address any issues

---

## Required Documents Checklist

### Employee Must Provide

**Day 1 (Critical)**:
- [ ] I-9 documentation (unexpired, original documents)
- [ ] Government-issued photo ID
- [ ] Social Security card or birth certificate

**Week 1**:
- [ ] Void check or bank account information
- [ ] Dependent documentation (if enrolling in benefits)
  - [ ] Marriage certificate
  - [ ] Birth certificates for children
- [ ] Previous employer 401(k) rollover information (if applicable)

### HR Provides to Employee

**Day 1**:
- [ ] Employee handbook (physical or digital)
- [ ] Benefits summary
- [ ] Company policies
- [ ] Organizational chart
- [ ] Building access badge

**Week 1**:
- [ ] Health insurance cards
- [ ] 401(k) enrollment confirmation
- [ ] Training certificates
- [ ] Parking pass (if applicable)

---

## System Access and Training

### Day 1
- [ ] Email account activated
- [ ] Employee portal access
- [ ] Building badge activated

### Week 1
- [ ] All required system access provisioned
- [ ] Training platform access
- [ ] HRIS self-service portal
- [ ] Expense system (if applicable)

---

## Onboarding Milestones

| Milestone | Timeline | Owner | Completion |
|-----------|----------|-------|------------|
| Paperwork complete | Day 1 | HR | [ ] |
| Benefits enrolled | Week 1 | Employee | [ ] |
| Compliance training done | Week 1 | Employee | [ ] |
| Benefits confirmed | Week 2 | HR | [ ] |
| 30-day review | Week 4 | Manager | [ ] |
| 60-day review | Week 8 | Manager | [ ] |
| 90-day review | Week 12 | Manager | [ ] |

---

## Success Metrics

We measure onboarding success by:
- **Paperwork completion**: 100% by Day 1
- **Benefits enrollment**: 100% by end of Week 1
- **Training completion**: 100% by end of Week 1
- **30-day retention**: Target 98%
- **90-day performance**: Meet or exceed expectations
- **Employee satisfaction**: Onboarding survey score > 4.5/5

---

## Communication Schedule

### Employee Receives

- **Week before start**: Welcome email with logistics
- **Day 1**: Daily recap email with next steps
- **Day 2-5**: Benefits enrollment reminders
- **End Week 1**: Completion confirmation
- **Week 2**: Benefits confirmation meeting invite
- **Week 3**: Mid-point check-in invite
- **Week 4**: 30-day review meeting invite

### Manager Receives

- **Week before start**: New hire notification
- **Day 1**: Onboarding kickoff confirmation
- **End Week 1**: Training completion notification
- **Week 4**: 30-day review reminder

---

## Troubleshooting

### Common Issues

**Issue**: I-9 documentation not acceptable
**Solution**: Provide list of acceptable documents, reschedule if needed. Cannot start work without valid I-9.

**Issue**: Benefits enrollment deadline missed
**Solution**: Special enrollment exception requires VP approval. Otherwise, must wait for open enrollment.

**Issue**: Compliance training not completed
**Solution**: System access may be limited. Required completion before Week 2.

**Issue**: First paycheck incorrect
**Solution**: Contact payroll immediately. Correction processed in next pay cycle.

---

## Contacts

- **General HR**: hr@company.com | ext. 1234
- **Benefits**: hr-benefits@company.com | ext. 1235
- **Payroll**: payroll@company.com | ext. 1236
- **IT Support**: it-support@company.com | ext. 4357
- **Training**: training-support@company.com | ext. 1237

---

**Last Updated**: 2025-11-01
**Owner**: HR Department
**Next Review**: 2026-02-01
**Version**: 2.0.0
**Estimated Time**: Week 1: 8-10 hours | Weeks 2-4: 2-3 hours
